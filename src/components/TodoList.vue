<template>
    <div class="flex flex-col gap-3">
        ...
        <div v-if="filters" class="flex flex-row justify-end gap-4">
            <span> Filter: </span>
            <button class="filter-button" :class="{ active: setFilter === 'finished' }"
                @click="toggleFilter('finished')"
                :aria-label="setFilter === 'finished' ? 'show all todos, current filter is set to finished' : 'show only finished todos'">
                finished
            </button>
            <button class="filter-button" :class="{ active: setFilter === 'unfinished' }"
                @click="toggleFilter('unfinished')"
                :aria-label="setFilter === 'unfinished' ? 'show all todos, current filter is set to unfinished' : 'show only unfinished todos'">
                unfinished
            </button>
        </div>
        ...
        <ul class="w-full flex flex-col gap-2"
            :aria-label="`${setFilter === 'none' ? 'all' : setFilter === 'finished' ? 'finished' : 'unfinished'} todos`">
            <li class="row-container" :class="{ done: todo.done }" v-for="(todo, index) in filteredTodos" :todo="todo"
                :key="todo.id" @keypress.shift.up="moveUp(index, todo)" @keypress.shift.down="moveDown(index, todo)">
                <span class="sr-only">
                    ({{ todo.done ? 'finished' : 'unfinished' }}) todo
                </span>

                <input type="checkbox" v-model="todo.done" class="cursor-pointer" @change="emitUpdate" ref="row-start"
                ...
                <input type="text" class="border rounded px-2 description-editable" v-model="editinigDescription"
                    ...
                    :aria-label="'description of todo ' + index + ' press enter to confirm, escape to cancel'" />
                ...
                <template v-else>
                    <button class="cursor-pointer" @click="doneEditMode()" aria-label="confirm your input">save</button>
                ...
                <button class="cursor-pointer rotate-90" :class="{ invisible: index === 0 }"
                    @click="moveUp(index, todo)" :ref="'button-up-' + todo.id" aria-label="move entry one up"
                    ref="upButton">
                    {{ "<" }} </button>

                        <button class="cursor-pointer rotate-90" :class="{ invisible: index === todos.length - 1 }"
                            aria-label="move entry one down" @click="moveDown(index, todo)" ref="downButton"
                            :ref="'button-down-' + todo.id">
                            {{ ">" }}
                        </button>

                        <button class="cursor-pointer" @click="deleteTodo(index, todo)" aria-label="remove entry">x</button>
            </li>
        </ul>
        ...
        export default {
            ...
            methods: {
                ...
                deleteTodo(index, todo) {
                    ...
                    this.announce(`deleted entry at position ${index + 1}, you are now on the next entry`)
                },
                moveUp(index, todo) {
                    ...
                    this.announce(`moved entry up from position ${index + 1} to ${index}` + (index <= 1 ? 'reached top, you are now on the move down button' : ''))
                },
                moveDown(index, todo) {
                    ...
                    this.announce(`moved entry down from position ${index + 1} to ${index + 2}` + (index + 2 >= this.todos.length ? 'reached bottom, you are now on the move up button' : ''))
                },
                emitUpdate() {
